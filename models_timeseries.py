...
class FCNBaseline(nn.Module):
    ...
    def __init__(self, in_channels: int, num_pred_classes: int = 1, x=None) -> None:
        super().__init__()
...
#         ])
        ...
        if x is not None:
#             x = preproc.add_magnitudes(x)
            print(x.shape)
        
        layers = []
        
        layer = ConvBlock(in_channels, 128, 20, 10)
        layers.append(layer)
        if x is not None:
            x = layer(x)
            print(x.shape)
            
        layers.append(nn.Dropout(p=drop1))
        
        layer = ConvBlock(128, 256, 15, 5)
        layers.append(layer)
        if x is not None:
            x = layer(x)
            print(x.shape)
    ...
    def forward(self, x: torch.Tensor, verbose=False) -> torch.Tensor:  # type: ignore
#         x = preproc.add_magnitudes(x)
